        ...
        for (size_t i{ 1 }; i < m_gridHeight - 1; ++i) {
            for (size_t j{ 1 }; j < m_gridWidth - 1; ++j) {
                ...
                (*resultPtr)(i, j) = 2 * (*previousResultPtr)(i, j) - (*resultPtr)(i, j) + m_timeStep * m_timeStep *
                        ...
                        (((*previousResultPtr)(i, j + 1) - (*previousResultPtr)(i, j)) *
                        (phaseSpeed(i - 1, j) + phaseSpeed(i, j)) +
                        ((*previousResultPtr)(i, j - 1) - (*previousResultPtr)(i, j)) *
                        (phaseSpeed(i - 1, j - 1) + phaseSpeed(i, j - 1))) /
                        (2 * m_xGridStep * m_xGridStep) +
                        (((*previousResultPtr)(i + 1, j) - (*previousResultPtr)(i, j)) *
                        (phaseSpeed(i, j - 1) + phaseSpeed(i, j)) +
                        ((*previousResultPtr)(i - 1, j) - (*previousResultPtr)(i, j)) *
                        (phaseSpeed(i - 1, j - 1) + phaseSpeed(i - 1, j))) /
                        (2 * m_yGridStep * m_yGridStep));
            }
        }
